- **登录增强 + 登录页主题切换修复（2026-08-23 23:06）**：①登录页明暗主题切换失效——切换逻辑只在 app.js,而登录页只加载 helpers.js(仅 FOUT-guard 读 data-theme)；已把 toggleTheme+initThemeToggle 迁移到 helpers.js(全站可用),登录页点击即切换。②登录可选项：支持用户名或邮箱登录(AuthController findByLogin 先email后username,表单字段 email→login)；记住我/自动登录(7天)——users 表加 remember_token(哈希)+remember_expires 列(ensureUserRemember 自迁移,overlay 安全),登录设 HttpOnly/Secure/SameSite cookie,DB 只存哈希+过期,AuthMiddleware 从 cookie 恢复 session,登出注销 token+清 cookie。记住我时长做成常量(7天可改)
- **在线测试重定向重复点击无请求（2026-08-23 22:34）**：首页「在线测试」选 redirect 时，首次点击成功加载图片，第二次点击只闪一下、devtools 无任何请求。根因：服务端 Response::redirect 的 302 响应未带 Cache-Control，浏览器按启发式规则缓存 302+图片；再次设置相同 src 命中缓存不发起请求。修复(纯前端)：redirect 分支给 Image src 加一次性缓存爆破 query(_=Date.now()+Math.random())，保证每次唯一；json 分支(在线测试+hero 试试手气)用 fetch(url,{cache:'no-store'})。仅改 app.js
- **API 文档首屏空白修复 + 取消自定义字体（2026-08-23 22:05）**：①首屏空白——.doc-pane 默认 display:none 但首个 pane 无初始 .active，须点击导航才显示；已给 endpoint-random 加 active 类(首屏即显示随机图片)。②取消自定义字体——移除 fonts.css 引用(8 视图)+删除 fonts.css 与 10 个 woff2 资源，--font/--font-mono/--font-display 改为系统字体栈(system-ui/ui-monospace)，Nunito/JetBrains Mono/ZCOOL KuaiLe 全部下线
- **API 文档侧边栏改为 tab 切换（2026-08-23 21:55）**：首页 API 文档 4 个 endpoint 原全部纵向堆叠，左侧快捷跳转仅锚点。改为点击左侧导航切换显示对应 endpoint——home.php 侧栏链接加 data-doc-target，每个 .doc-endpoint 加 .doc-pane 类(默认第一个 random active)；app.js 加 initDocsNav()(preventDefault + 同步 nav/pane 的 .active)；style.css 加 .doc-pane display:none + .active display:block。事件委托用 document 层(无容器 id)
- **设置页滚到底保存条贴底（2026-08-23 19:17，滚到底难看的二次修复）**：上一步 docked 保存条后滚到最底仍出现空档+孤悬的「查看操作日志」——①该入口链接放在所有 section 之后，sticky 条(包含块=section)到 section 边缘就停，链接+main-content 32px 底 padding 露在其下；②section 未撑到视口高，sticky 停点不到真底部。修复：入口链接上移至顶部工具条 + admin_header() 加可选 body 类(page-settings) + .page-settings .main-content flex-column 且 .settings-group flex:1 0 auto 撑满；其余 admin 页单参数调用零影响
