...
- **字体本地化**（用户反馈 Google Fonts 加载失败）：下载 ZCOOL KuaiLe/Nunito/JetBrains Mono 全部 104 个 woff2 分片（1.5MB）到 src/assets/fonts/，生成 fonts.css（@font-face + unicode-range，108KB）；8 个页面（home/admin layout/login/install 5页）的 Google Fonts 链接全部替换为本地 /assets/fonts/fonts.css；国内部署不再依赖外网字体
